  ...
  const initChatbot = () => {
    ...
    if (restartMenuBtn && confirmModal) {
      ...
      confirmYes.addEventListener("click", () => {
        ...
        addMessage(
          "bot",
          "Histórico limpo! 👋 Como posso ajudar a eternizar o seu momento hoje?",
        );
        confirmModal.classList.add("hidden");
      });
    }
    ...
    async function loadHistory() {
      ...
      try {
        const response = await fetch(
          `backend/get_messages.php?conversation_id=${conversationId}`,
        );
    ...
    chatForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        botSound
          .play()
          .catch((e) => console.log("Áudio bloqueado temporariamente"));
      } catch (error) {
